  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="alert alert-info">
        Tab ini membaca ulang folder output dari HP, jadi tetap bisa dipakai
        walau kamu belum menjalankan merge di sesi ini.
      </div>
      <div class="btn-row">
        <button class="btn btn-outline" onclick="loadEmailTab()">🔄 Muat / Refresh Daftar File</button>
      </div>
      <div id="email-tab-body" class="mt12"></div>
    </div>
  </div>
</div>
...
function runSendEmail(folders, statusElId, btnRowElId, retryFnName) {
  ...
  const logId  = statusElId + '-log';
  const progId = statusElId + '-progress';
  statusEl.innerHTML =
    `<div class="alert alert-info"><span class="spinner"></span> Menyiapkan email...</div>
     <div class="progress-wrap" style="margin-top:8px">
       <div id="${progId}" class="progress-bar" style="width:0%"></div>
     </div>
     <div id="${logId}" style="margin-top:8px;font-size:.82rem"></div>`;

  let url = '/api/send-email-stream';
  ...
  es.onmessage = function(e) {
    ...
    const alertBox = statusEl.querySelector('.alert');

    if (t === 'email_start') {
      if (alertBox) alertBox.innerHTML = `<span class="spinner"></span> Mengirim ${d.total} email...`;
    }
      ...
      if (alertBox) {
        alertBox.className = 'alert ' + (d.ok > 0 ? (d.fail > 0 ? 'alert-warn' : 'alert-success') : 'alert-error');
        alertBox.innerHTML = d.ok > 0
...
function renderEmailTab() {
  ...
  folders.forEach(folder => {
    ...
    html += `
      <div class="card" style="margin-bottom:10px">
        ...
        <div class="card-body">
          <div class="file-list">
            ${entries.map(([key, nama, serial]) =>
              `<div class="file-item">📄 ${key}.pdf <span class="dim">— ${nama}</span> <span style="color:var(--teal)">SN:${serial}</span></div>`
            ).join('')}
          </div>
        </div>
      </div>`;
  ...
  html += `
    <div id="email-tab-status" class="mt12"></div>
...
function sendSelectedEmails() {
  ...
  if (checked.length === 0) {
    const el = document.getElementById('email-tab-status');
...
# ── Email Scan (mandiri, baca ulang dari disk) ──────────────
...
def api_email_scan():
    ...
    slim = {folder: [[k, n, s] for k, n, s, _ in entries]
...
def api_send_email_stream():
    ...
    selected_folders = [f for f in folders_param.split(",") if f] if folders_param else None

    # Prioritaskan hasil merge yang masih ada di memori (alur tab Merge);
    # kalau tidak ada, baca ulang dari disk (dipakai tab Kirim Email mandiri).
    ...
    if not summary:
        def gen_err():
            yield f"data: {json.dumps({'type':'error','data':{'msg':'Tidak ada file untuk dikirim'}})}\n\n"
    ...
    def worker():
        try:
            core.do_send_emails(summary, cfg, cb)
        except Exception as e:
            q.put({"type": "error", "data": {"msg": str(e)}})
        finally:
            q.put(None)
