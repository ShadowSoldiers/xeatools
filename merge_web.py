...
<div class="tabs">
  ...
  <div class="tab" onclick="showTab('email')">📧 Kirim Email</div>
...
<div id="tab-email" class="section">
  ...
  <div class="card">
    <div class="card-header">📧 Kirim Email — File Hasil Merge</div>
    <div class="card-body">
      <div class="alert alert-info">
        Tab ini membaca ulang folder output dari HP, jadi tetap bisa
        dipakai walau kamu belum menjalankan merge di sesi ini.
      </div>
      <div class="btn-row">
        <button class="btn btn-outline" onclick="loadEmailTab()">🔄 Muat / Refresh Daftar File</button>
      </div>
      <div id="email-tab-body" class="mt14"></div>
    </div>
  </div>
...
function sendEmails() {
  ...
  fetch('/api/send-email', {method:'POST'})
    .then(r => r.json())
    .then(r => {
      renderEmailResult(r, 'email-status', 'email-btn-row', sendEmails);
    })
    .catch(() => {
      document.getElementById('email-status').innerHTML =
        '<div class="alert alert-error">Gagal terhubung ke server.</div>' +
        retryButtonHtml('sendEmails()');
      document.getElementById('email-btn-row').classList.remove('hidden');
    });
}

// ── Helper hasil kirim email (dipakai di 2 tempat: tab Jalankan & tab Kirim Email) ──
function retryButtonHtml(fnCall) {
  return `<div class="btn-row"><button class="btn btn-primary" onclick="${fnCall}">🔄 Coba Lagi</button></div>`;
}

function renderEmailResult(r, statusElId, btnRowElId, retryFn) {
  const statusEl = document.getElementById(statusElId);
  const isAuthFail = (r.detail || []).some(([t, ok, msg]) =>
    !ok && /login gagal|app password/i.test(msg || ''));

  let html = '';
  if (r.ok > 0) {
    html += `<div class="alert alert-success">✓ ${r.ok} email terkirim.</div>`;
  }
  if (r.fail > 0) {
    html += `<div class="alert alert-error">✗ ${r.fail} email gagal terkirim.</div>`;
    if (isAuthFail) {
      html += `<div class="alert alert-warn">🔑 Sepertinya App Password Gmail salah/kadaluarsa.
                Cek dan perbarui di tab <b>Konfigurasi</b>, lalu coba lagi.</div>`;
    }
  }
  if (r.error) {
    html += `<div class="alert alert-error">${r.error}</div>`;
  }
  html += (r.detail || []).map(([t, ok, msg]) =>
    `<div class="file-item">${ok ? '✓' : '✗'} [${t}] ${msg}</div>`
  ).join('');

  if (r.fail > 0 || r.error) {
    html += retryButtonHtml(retryFn.name + '()');
  }

  statusEl.innerHTML = html || '<div class="alert alert-warn">Tidak ada yang dikirim.</div>';

  // Tombol "Ya, Kirim Email" / "Kirim Email Terpilih" tetap disembunyikan kalau semua
  // sudah sukses (tidak ada yang perlu dicoba lagi); tampil lagi kalau ada yang gagal.
  const btnRow = document.getElementById(btnRowElId);
  if (btnRow) {
    if (r.fail > 0 || r.error) btnRow.classList.remove('hidden');
    else btnRow.classList.add('hidden');
  }
}
...
// ── Tab: Kirim Email (mandiri, baca ulang dari disk) ────────
let emailScanData = {};

function loadEmailTab() {
  const body = document.getElementById('email-tab-body');
  body.innerHTML = '<div class="dim">Memuat daftar file...</div>';
  fetch('/api/email-scan').then(r => r.json()).then(data => {
    emailScanData = data.summary || {};
    renderEmailTab();
  }).catch(() => {
    body.innerHTML = '<div class="alert alert-error">Gagal memuat daftar file.</div>';
  });
}

function renderEmailTab() {
  const body = document.getElementById('email-tab-body');
  const folders = Object.keys(emailScanData).sort();

  if (folders.length === 0) {
    body.innerHTML = '<div class="alert alert-warn">Belum ada file hasil merge yang siap dikirim.</div>';
    return;
  }

  let html = '';
  folders.forEach(folder => {
    const entries = emailScanData[folder];
    html += `
      <div class="card" style="margin-bottom:10px">
        <div class="card-header" style="display:flex;align-items:center;gap:8px;background:#0891b2">
          <label style="display:flex;align-items:center;gap:8px;margin:0;color:#fff;font-weight:600;cursor:pointer">
            <input type="checkbox" class="email-folder-chk" data-folder="${folder}" checked style="width:16px;height:16px">
            ${folder} — ${entries.length} file
          </label>
        </div>
        <div class="card-body">
          <div class="file-list">
            ${entries.map(([key, nama]) =>
              `<div class="file-item">📄 ${key}.pdf <span class="dim">— ${nama}</span></div>`
            ).join('')}
          </div>
        </div>
      </div>`;
  ...
  html += `
    <div id="email-tab-status" class="mt8"></div>
    <div class="btn-row" id="email-tab-btn-row">
      <button class="btn btn-success" onclick="sendSelectedEmails()">📧 Kirim Email Terpilih</button>
    </div>`;
...
function sendSelectedEmails() {
  const checked = Array.from(document.querySelectorAll('.email-folder-chk:checked'))
                        .map(el => el.dataset.folder);

  if (checked.length === 0) {
    document.getElementById('email-tab-status').innerHTML =
      '<div class="alert alert-warn">Pilih minimal satu tipe layanan untuk dikirim.</div>';
    return;
  }

  document.getElementById('email-tab-btn-row').classList.add('hidden');
  document.getElementById('email-tab-status').innerHTML =
    '<div class="alert alert-info"><span class="spinner"></span>  Mengirim email...</div>';
  ...
  fetch('/api/send-email', {
    ...
    .then(r => r.json())
    .then(r => {
      renderEmailResult(r, 'email-tab-status', 'email-tab-btn-row', sendSelectedEmails);
    })
    .catch(() => {
      document.getElementById('email-tab-status').innerHTML =
        '<div class="alert alert-error">Gagal terhubung ke server.</div>' +
        retryButtonHtml('sendSelectedEmails()');
      document.getElementById('email-tab-btn-row').classList.remove('hidden');
    });
}
...
@app.route("/api/email-scan")
def api_email_scan():
    """Baca ulang folder output dari disk untuk tab Kirim Email (mandiri)."""
    cfg = core.load_config()
    summary = core.scan_email_candidates(cfg["output_dir"])
    # Path -> str, sisakan (key, nama) saja untuk tampilan
    slim = {folder: [[k, n] for k, n, _ in entries]
            for folder, entries in summary.items()}
    return jsonify({"summary": slim})

@app.route("/api/send-email", methods=["POST"])
def api_send_email():
    cfg = core.load_config()
    body = request.get_json(silent=True) or {}
    selected_folders = body.get("folders")  # None = pakai semua

    # Prioritaskan hasil merge yang masih ada di memori (sesi berjalan),
    # jika tidak ada baca ulang dari disk (tab Kirim Email mandiri).
    ...
    if selected_folders is not None:
        summary = {k: v for k, v in summary.items() if k in selected_folders}

    if not summary:
        return jsonify({"ok": 0, "fail": 0, "detail": [], "error": "Tidak ada file untuk dikirim"})

    email_result = core.do_send_emails(summary, cfg)
