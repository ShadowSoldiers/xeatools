...
def scan_email_candidates(output_dir: str) -> dict:
    ...
    {folder_name: [(key, nama, Path)]} yang siap dikirim lewat do_send_emails().
    Dipakai oleh tab 'Kirim Email' agar tidak bergantung pada hasil merge
    yang masih tersimpan di memori (mis. setelah server di-restart).
    """
    ...
    for folder_name in known_folders:
        ...
        # Baca daftar_pelanggan_*.txt untuk mapping key -> nama
        nama_map = {}
        ...
        for pdf in sorted(folder_path.glob("*.pdf")):
            key  = pdf.stem
            nama = nama_map.get(key, "-")
            entries.append((key, nama, pdf))
